...
# ╔═╡ 29e46689-d921-4ed5-afff-af01d31f8522
begin
	# Animation settings
	t = 0:0.02:30
	k = 4
...
begin 
	...
	function dp(θ₁, θ₂, ω₁, ω₂, m₁, m₂, g, L₁, L₂)
	    ...
	    dω₂ = (
	        L₂*m₂*ω₂^2*sin(2*θ₂-2*θ₁) + 2*L₁*m₂*ω₁^2*sin(θ₂-θ₁) + 
			2*L₁*m₁*ω₁^2*sin(θ₂-θ₁) + g*m₂*(sin(θ₂-2*θ₁)+sin(θ₂)) + 
			g*m₁*(sin(θ₂-2*θ₁)+sin(θ₂))) / (L₂*m₂*(cos(2*(θ₂-θ₁))-1) - 2*L₂*m₁)
	    return (dω₁, dω₂)
	end
	...
	function dpwf(θ₁, θ₂, ω₁, ω₂, m₁, m₂, g, L₁, L₂, μ₁, μ₂)
	    ...
	    dω₂ = (
	        L₂*m₂*ω₂*(cos(2*(θ₂-θ₁))-1)*μ₂ - 2*L₁*m₁*ω₁*cos(θ₂-θ₁)*μ₂ - 
			2*L₂*m₁*ω₂*μ₂ + 2*L₁*m₂*ω₁*cos(θ₂-θ₁)*μ₁ - 
			L₂*m₂^2*ω₂^2*sin(2*θ₂-2*θ₁) - 2*L₁*m₂^2*ω₁^2*sin(θ₂-θ₁) - 
	        2*L₁*m₁*m₂*ω₁^2*sin(θ₂-θ₁) - g*m₂^2*(sin(θ₂-2*θ₁)+sin(θ₂)) 
			- g*m₁*m₂*(sin(θ₂-2*θ₁)+sin(θ₂))
...
function quiver_plot_params(parameters::Vector)
	...
	return combine() do Child
		...
		inputs = [
			md""" $(name): $(
				Child(name, Slider(0.1:pi/5:10pi, default=2pi, show_value=true))
			)"""
...
@bind spfp quiver_plot_params(["l_θ_bound", "r_θ_bound", "l_ω_bound", "r_ω_bound", "θ_step", "ω_step"])

# ╔═╡ dbbba9bd-62ef-4c99-b7bb-49493f42a1fb
@bind dpfp quiver_plot_params(["θ", "ω", "θ_step", "ω_step", "θ_other", "ω_other"])

# ╔═╡ e1834527-30fd-4f8a-ae0d-208b48eaf2e6
function field_plot_single(
	θbounds=[3pi/2, 3pi/2], 
	ωbounds=[3pi/2, 3pi/2], 
...
mesh = [[i, j] 
	...
	fieldp = quiver(θf, ωf, quiver=(lens .* ωf, lens .* slopes), line=(2, 0.6))
	plot!(fieldp, θ, ω, line=(2.5), label="θ'(t) = θ(t)", legend=:topright)
	plot!(fieldp, framestyle=:origin, aspect_ratio=1, dpi=200)
	plot!(fieldp, xaxis=("θ", (-θbounds[1]+pi/2, θbounds[2]-pi/2)))
	plot!(fieldp, yaxis=("ω", (-ωbounds[1]+pi/2, ωbounds[2]-pi/2)))
end
...
function field_plot_double(solution, func, param_list, set=1,
	bounds=[3pi/2, 3pi/2], steps=[pi/5, pi/5], other_dims=[1, 1], kh=0.9)
	
	θ₁, θ₂ = @. (vec_to_list(solution.u, 1), vec_to_list(solution.u, 2))
	ω₁, ω₂ = @. (vec_to_list(solution.u, 3), vec_to_list(solution.u, 4))
	sets = [[θ₁, ω₁], [θ₂, ω₂]]
	
	mesh = [
		[i, j] 
		for i=-bounds[1]:steps[1]:bounds[1] 
		for j=-bounds[2]:steps[2]:bounds[2]
	]
	
	point_extr = point -> [
		[point[1], other_dims[1], point[2], other_dims[2]], 
		[other_dims[1], point[1], other_dims[2], point[2]]
	]
	
	slopes = [func(point_extr(point)[set]..., param_list...) for point in mesh]
	fs = @. (vec_to_list(mesh, 1), vec_to_list(mesh, 2))
	our_slopes = vec_to_list.(slopes, set)
	lens = @. (1 / sqrt(fs[2]^2 + our_slopes ^ 2) * kh)
	

	fieldp2 = quiver(
		fs..., 
		quiver=(lens .* fs[2], lens .* our_slopes), line=(3, 0.8))
	# plot!(fieldp2, sets[set]..., line=(2), label="θ'(t) = θ(t)", legend=:topright)
	plot!(fieldp2, framestyle=:origin, size=(1000, 1000), aspect_ratio=1, dpi=200)
	plot!(fieldp2, xaxis=("θ", (-bounds[1]+pi/2, bounds[1]-pi/2)))
	plot!(fieldp2, yaxis=("ω", (-bounds[2]+pi/2, bounds[2]-pi/2)))
end
...
# ╔═╡ 26c4083b-75b0-4724-96f6-22a103c2d686
begin
